<template>
  <div class="anime-page">
    ...
    <div class="search-area">
      ...
      <el-button type="primary" @click="xyyiliaoyhYhbinglijilvListJson()">搜索</el-button>
    </div>
...
<script>
  const xyyiliaoyhYhbinglijilvList = {
    components: {
    },
    data() {
      return {
    ...
    methods: {
      xyyiliaoyhDeleteYhbinglijilvJson(id) {
        var _this = this
        console.log(id)
        this.axios.post("/xyyiliaoyhDeleteYhbinglijilvJson?id=" + id, {}).then(res => {
          if (res.data.message == '删除用户病例记录成功') {
            ...
            _this.yhbinglijilvListAll = _this.yhbinglijilvListAll.filter(item =>{
              return item.id != id
            })
      ...
      toXyyiliaoyhUpdateYhbinglijilv(id) {
        this.$router.push({path:"/htMenu/xyyiliaoyhUpdateYhbinglijilv",query:{id:id}})
      },
      xyyiliaoyhYhbinglijilvListJson() {
        var _this = this
        this.axios.post("/xyyiliaoyhYhbinglijilvListJson?search="+this.search,{}).then(res => {
          _this.yhbinglijilvListAll  = res.data.yhbinglijilvs
          _this.goPage(0)
        })
      },
      goPage(pageIndex){
        this.pageNum = Math.ceil(this.yhbinglijilvListAll.length / this.pageSize)
        //最少都有一页
        if (this.pageNum == 0){
          this.pageNum = 1
        }
        this.showYhbinglijilvArr = this.yhbinglijilvListAll.slice(pageIndex*this.pageSize,(pageIndex+1)*this.pageSize)
    ...
    created() {
      var _this = this
      this.xyyiliaoyhYhbinglijilvListJson()
    },
    //可以操作dom了
    mounted() {
