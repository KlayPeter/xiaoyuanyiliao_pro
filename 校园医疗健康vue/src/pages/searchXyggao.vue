    ...
    <div class="search-section">
      <div class="search-box">
        <el-input
                ...
                placeholder="请输入关键词"
                class="search-input"
                @keyup.enter="searchXyggaoJson()"
        >
          <template #prefix>
            <i class="el-icon-search"></i>
          </template>
        </el-input>
        <el-button type="primary" size="large" @click="searchXyggaoJson()">搜索</el-button>
      </div>
    </div>

    <div class="anime-list-section">
      <div class="section-header">
        <h3 class="section-title">校园公告列表</h3>
      </div>
      <div class="anime-grid">
        <div v-for="(xyggao,index) in showXyggaoArr" :key="xyggao.id" class="anime-card">
          <router-link :to="'/qtMenu/xyggaoDetails?id='+xyggao.id" style="text-decoration: none;">
            <div class="gonggao-image">
              <el-image :src="'http://localhost:8080/xiaoyuanyiliaojiankang/static/upload/'+xyggao.img" fit="cover" />
            </div>
            <div class="gonggao-content">
              <h4 class="gonggao-title">{{xyggao.xytitle}}</h4>
...
<script>
  const searchXyggao = {
    ...
    methods:{
      searchXyggaoJson(){
        var _this = this
        this.axios.post("/searchXyggaoJson?search=" + _this.search,{}).then(res=>{
