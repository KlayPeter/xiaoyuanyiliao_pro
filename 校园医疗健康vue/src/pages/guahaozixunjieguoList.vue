<template>
    <div class="anime-page">
        ...
        <div class="search-area">
            ...
            <el-button type="primary" @click="guahaozixunjieguoListJson()">搜索</el-button>
        </div>
            ...
            <el-table-column fixed="right" label="操作" min-width="120">
                <template #default="scope">
                    <el-button class="anime-btn" size="small"
                               @click.prevent="toUpdateGuahaozixunjieguo(scope.row.id)"
                    >修改</el-button>
                    <el-button class="anime-btn-cancel" size="small"
                               @click.prevent="deleteGuahaozixunjieguoJson(scope.row.id)"
                    >删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
  const guahaozixunjieguoList = {
    ...
    methods: {
      deleteGuahaozixunjieguoJson(id) {
        var _this = this
        console.log(id)
        this.axios.post("/deleteGuahaozixunjieguoJson?id=" + id, {}).then(res => {
          if (res.data.message == '删除挂号咨询结果成功') {
            ...
            _this.guahaozixunjieguos = _this.guahaozixunjieguos.filter(item =>{
              return item.id != id
            })
      ...
      toUpdateGuahaozixunjieguo(id) {
        this.$router.push({path:"/htMenu/updateGuahaozixunjieguo",query:{id:id}})
      },
      guahaozixunjieguoListJson() {
        var _this = this
        this.axios.post("/guahaozixunjieguoListJson?search="+this.search,{}).then(res => {
          _this.guahaozixunjieguos  = res.data.guahaozixunjieguos
          _this.goPage(0)
        })
      },
      goPage(pageIndex){
        this.pageNum = Math.ceil(this.guahaozixunjieguos.length / this.pageSize)
        //最少都有一页
        if (this.pageNum == 0){
          this.pageNum = 1
        }
        this.showGuahaozixunjieguoArr = this.guahaozixunjieguos.slice(pageIndex*this.pageSize,(pageIndex+1)*this.pageSize)
    ...
    created() {
      var _this = this
      this.guahaozixunjieguoListJson()
    },
    //可以操作dom了
    mounted() {
  ...
  export default guahaozixunjieguoList
</script>
