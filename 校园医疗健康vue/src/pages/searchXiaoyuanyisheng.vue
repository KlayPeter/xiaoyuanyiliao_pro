    ...
    <div class="search-section">
      <div class="search-box">
        <el-input
                ...
                placeholder="请输入关键词"
                class="search-input"
                @keyup.enter="searchXiaoyuanyishengJson()"
        >
          <template #prefix>
            <i class="el-icon-search"></i>
          </template>
        </el-input>
        <el-button type="primary" size="large" @click="searchXiaoyuanyishengJson()">搜索</el-button>
      </div>
    </div>

    <div class="anime-list-section">
      <div class="section-header">
        <h3 class="section-title">校园医生列表</h3>
      </div>
      <div class="anime-grid">
        <div v-for="(xiaoyuanyisheng,index) in showXiaoyuanyishengArr" :key="xiaoyuanyisheng.id" class="anime-card">
          <router-link :to="'/qtMenu/xiaoyuanyishengDetails?id='+xiaoyuanyisheng.id" style="text-decoration: none;">
            <div class="gonggao-image">
              <el-image :src="'http://localhost:8080/xiaoyuanyiliaojiankang/static/upload/'+xiaoyuanyisheng.img" fit="cover" />
            </div>
            <div class="gonggao-content">
              <h4 class="gonggao-title">{{xiaoyuanyisheng.username}}</h4>
...
<script>
  const searchXiaoyuanyisheng = {
    ...
    methods:{
      searchXiaoyuanyishengJson(){
        var _this = this
        this.axios.post("/searchXiaoyuanyishengJson?search=" + _this.search,{}).then(res=>{
    ...
    created() {
      var _this = this
      this.searchXiaoyuanyishengJson()
    },
    //可以操作dom
    mounted() {
  ...
  export default searchXiaoyuanyisheng
</script>
