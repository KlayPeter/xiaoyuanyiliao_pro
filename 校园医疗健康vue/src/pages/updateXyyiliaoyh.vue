<template>
  <div class="anime-page">
    <div class="anime-card">
      <h2 class="page-header">修改校园用户</h2>
      <el-form label-width="100px" class="form">
        ...
        <el-form-item class="form-footer">
          <el-button type="primary" size="large" class="anime-btn" @click="updateXyyiliaoyhactJson()">
            ✨ 修改
          </el-button>
...
<script>
  const updateXyyiliaoyh = {
    ...
    methods: {
      ...
      updateXyyiliaoyhactJson() {
        var _this = this
        this.axios.post("/updateXyyiliaoyhactJson", _this.xyyiliaoyh).then(res => {
          if (res.data.message == '修改校园用户成功') {
            _this.$toast.success("修改校园用户成功",{
              position: "top",
              pauseOnHover: false,
              duration: 2000
            })
    ...
    created() {
      var _this = this
      this.axios.post("/updateXyyiliaoyhJson?id="+_this.$route.query.id).then(res=>{
        _this.xyyiliaoyh = res.data.xyyiliaoyh
      })
    },
    //可以操作dom了
    mounted() {
  ...
  export default updateXyyiliaoyh
</script>
<style scoped>
