<template>
	...
	<view>
		<form @submit="xyyiliaoyhUpdateXyyiliaopaibanyuyueactJson">
			<view class="form biaodan">
						...
						<input maxlength="-1" v-model="xyyiliaopaibanyuyue.jianyaowenti" class="input" placeholder="请输入简要问题描述" />
							<view style="margin: 10px;">选择校园医生:</view>
							<picker :range="xiaoyuanyishengs" range-key="username" @change="changeXiaoyuanyisheng">
...
<script>
	export default {
		...
		methods: {
			...
			xyyiliaoyhUpdateXyyiliaopaibanyuyueactJson(e) {
				let _this = this
				uni.request({
					url: "xyyiliaoyhUpdateXyyiliaopaibanyuyueactJson",
					method: "POST",
		...
		onLoad(e) {
			...
			console.log(e)
			uni.request({
				url: "xyyiliaoyhUpdateXyyiliaopaibanyuyueJson?id=" + e.id,
				success: res => {
