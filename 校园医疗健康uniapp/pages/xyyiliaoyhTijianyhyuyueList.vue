    ...
    <view class="container">
        ...
        <view class="tab-bar">
            <view v-for="(item, index) in statusList" :key="index"
                ...
                <view style="width: 150rpx">{{ item.label }}</view>
            </view>
        </view>
            ...
            <view v-else v-for="(order, index) in orderList" :key="index" class="order-card">
                <view  style="padding: 15px;">
                    <view class="order-header">
                        <text class="order-id"> {{ order.xytijianshixiang }}</text>
                        <text class="order-status">{{ order.state }}</text>
                    </view>
                    <view class="order-content">
                        <view>位置: {{ order.address }}</view>
                        <view>时间: {{ order.shijian }}</view>
                        <view class="order-price" style="display: flex;align-items: center;">
                            <button v-if="order.state == '待前往'" @click="qianwang(index)" type="primary" size="mini">前往</button>
							<button v-if="order.state == '已出成绩'" @click="shanchu(index)" type="primary" size="mini">删除</button>
						</view>
                    </view>
                </view>                
            </view>
